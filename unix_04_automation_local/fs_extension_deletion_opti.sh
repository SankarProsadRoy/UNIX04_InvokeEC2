...
echo "zipping 10 days older log files..."
#sudo find $FS_NAME -xdev -type f -name '*.log*' -mtime +10 -size +5000 -exec gzip -3 {} \; 2>/dev/null
sudo find $FS_NAME -xdev -type f -name '*.log*' ! \( -name "*.gz" \) ! \( -name "*.Z" \) -mmin +1 -size +5000 -exec gzip -3 -S "_`date '+%Y%m%d%H%M'`.gz"  {} \; 2>/dev/null &
...
echo >>$logfile
#sudo find $FS_NAME -xdev -type f -name '*.log*' -mtime +10 -size +5000 -exec gzip -3 {} \; 2>/dev/null
sudo find $FS_NAME -xdev -type f -name '*.log*' -mmin +1 -size +5000 -exec gzip -3 -S "_`date '+%Y%m%d%H%M'`.gz"  {} \; >>$logfile 2>&1
FSACTUAL=`df -k $FS_NAME | awk -F"%" '{print $1}' | tail -1 | awk '{print substr($0,length($0)-3)}' |sed 's/^[ *\t]//'`       #  Checking FS Utilization
echo >>$logfile
